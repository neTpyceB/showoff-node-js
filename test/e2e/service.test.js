...
test('balancer routes real http requests across backend processes', async () => {
  const balancerPort = 5100 + Math.floor(Math.random() * 500);
  ...
  try {
    ...
    response = await fetch(`http://127.0.0.1:${balancerPort}/records/42`, {
      headers: { Connection: 'close' }
    });
    ...
    assert.equal(response.status, 200);
    assert.equal((await response.json()).loadBalancer.requestsTotal, 2);
  } finally {
    await stopProcess(balancer);
    await stopProcess(backendA);
    await stopProcess(backendB, 'SIGINT');
  }
});
